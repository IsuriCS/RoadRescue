...
public class ReportsController {
    public JsonArray getTicketList(Connection connection) throws SQLException, ClassNotFoundException {
        ResultSet rst = CrudUtil.executeQuery(connection, "SELECT t.id, t.customer_id, t.title, t.created_time, t.status, c.phone_number FROM road_rescue.customer_support_ticket t JOIN road_rescue.customer c ON t.customer_id = c.id WHERE t.status = 'pending'");
